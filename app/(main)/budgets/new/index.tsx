...
import { budgetAPI, formatCHF, formatPercent } from '../../../../lib/api';
...
export default function NewBudgetScreen() {
  ...
  // Mode édition ACTIF PAR DÉFAUT pour un nouveau budget
  ...
  // Initialiser les strings d'input au montage
  useEffect(() => {
  ...
  const createBudget = useCallback(async () => {
    ...
    try {
      ...
      // Navigation vers le budget créé (remplacer la page actuelle)
      router.replace(`/(main)/budgets/${res.data.id}` as any);
      Alert.alert('✅ Créé', 'Ton budget a été créé !');
    } catch (err: any) {
      console.error('[NewBudget] Erreur création:', err);
      Alert.alert('Erreur', err?.response?.data?.detail || 'Échec de la création');
    } finally {
      setSaving(false);
    }
  }, [editData, router]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            MODE ÉDITION (toujours actif pour un nouveau budget)
        ═══════════════════════════════════════════════════════════════════ */}
